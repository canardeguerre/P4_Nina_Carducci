...
element.wrap(`<div class='item-column mb-4${columnClasses}'></div>`)}else{console.error(`Columns should be defined as numbers or objects. ${typeof columns} is not supported.`)}},moveItemInRowWrapper(element){element.appendTo(".gallery-items-row")},responsiveImageItem(element){if(element.prop("tagName")==="IMG"){element.addClass("img-fluid")}},openLightBox(element,lightboxId){$(`#${lightboxId}`).find(".lightboxImage").attr("src",element.attr("src"));$(`#${lightboxId}`).modal("toggle")},prevImage(){let activeImageSrc=$(".lightboxImage").attr("src");let activeTag=$(".tags-bar span.active-tag").data("images-toggle");let imagesCollection=[];$(".item-column").each(function(){let img=$(this).children("img");if(!img.length)return;if(activeTag==="all"||img.data("gallery-tag")===activeTag){imagesCollection.push(img)}});let currentIndex=imagesCollection.findIndex(img=>img.attr("src")===activeImageSrc);let prevIndex=(currentIndex-1+imagesCollection.length)%imagesCollection.length;$(".lightboxImage").attr("src",imagesCollection[prevIndex].attr("src"))},nextImage(){let activeImageSrc=$(".lightboxImage").attr("src");let activeTag=$(".tags-bar span.active-tag").data("images-toggle");let imagesCollection=[];$(".item-column").each(function(){let img=$(this).children("img");if(!img.length)return;if(activeTag==="all"||img.data("gallery-tag")===activeTag){imagesCollection.push(img)}});let currentIndex=imagesCollection.findIndex(img=>img.attr("src")===activeImageSrc);let nextIndex=(currentIndex+1)%imagesCollection.length;$(".lightboxImage").attr("src",imagesCollection[nextIndex].attr("src"))},createLightBox(gallery,lightboxId,navigation){gallery.append(`
